# AI-Courses

面向中小学生的 AI 兴趣课程研究、产品设计与课程产物仓库。

## 当前产品目标

本仓库当前优先研发两条正式课程线，并配套一个快速展示 AI 创造力的入门演示课：

1. **小学高年级 AI 创造课**：低技术门槛，以角色、故事、互动作品、小游戏、轻量应用等为载体。
2. **初中 AI 数字创造课**：不以传统编程为前置，充分借助 AI，让学生通过自然语言、选择、修改、试玩和迭代完成可运行的游戏/应用/AI 作品。
3. **AI Wow Demo / 入门演示课**：在很短时间内让学生、家长和学校看到“原来孩子现在可以用 AI 做到这个”的成果，用于体验、招生活动、开放日及正式学期课导入。

小学低年级暂不作为第一优先正式学期产品；保留教师主导、低文字负担的启蒙工作坊方向，待高年级和初中产品跑通后验证。

## 核心定位

> 从“问 AI”走向“用 AI 创造”。

课程不是传统编程课的简化版，也不是 AI 工具功能说明课。目标是利用 AI 把复杂技术封装在背后，让学生用很简单的操作和自然语言获得过去通常需要较高技术门槛才能实现的创造能力。

**作品是明线，AI 使用能力是暗线。**

学生感受到的是：做游戏、做互动故事、做网站/轻应用、做会对话的角色、做自己的数字产品。

课程实际培养的是：表达需求、任务拆解、提问、判断、验证、迭代、审美、逻辑、信息甄别、合作与负责任使用 AI。

## 仓库结构

```text
AI-Courses/
├── AGENTS.md                         # 后续人/Agent 工作入口
├── README.md                         # 仓库总览
├── docs/
│   ├── PRODUCT_STRATEGY.md           # 总体课程产品战略
│   ├── COMPETITIVE_RESEARCH.md       # 竞品/案例研究及来源
│   ├── PRODUCT_COMPARISON.md         # 横向对比与机会空间
│   ├── DECISIONS.md                  # 已确定产品决策与待验证项
│   └── TOOL_AND_WOW_SCAN_2026-08-31.md # 当前低门槛 Wow 成果/工具扫描
└── courses/
    ├── demo/README.md                # 入门演示课产品定义
    ├── primary-upper/README.md       # 小学高年级正式课
    ├── middle-school/README.md       # 初中正式课
    └── primary-lower/README.md       # 小学低年级候选方向/暂缓原因
```

后续每条课程线自己的教案、教师手册、学生材料、作品模板、Demo Day 方案、工具配置、试课记录等，应继续放入对应课程目录，不要把所有课程产物混放在 `docs/` 或仓库根目录。

## 当前阶段

当前处于 **产品架构与课程原型阶段**。当前正式工作项是 `docs/TOOL_AND_WOW_SCAN_2026-08-31.md`：先验证 AI 已经把哪些过去复杂的数字创作能力压缩成自然语言和简单操作，再选择课程原型。

下一步不是先写满一学期教案，而是先验证：

- 哪些成果能做到“技术难度 2/10、作品观感 8/10”；
- 哪些工具在中国学校网络、设备、账号和未成年人使用约束下稳定可用；
- 哪个 45–60 分钟演示课最容易制造第一波“眼前一亮”；
- 小学高年级与初中正式课的 3–4 个能力跃迁节点如何设计；
- 普通信息科技/科学/综合实践教师经过较短培训能否稳定交付。

当前首轮 P0 机制包括：

1. 自然语言生成单文件 HTML 游戏 / 动画 / 3D 互动场景；
2. AI Agent 生成 HTML + 本地数据的轻应用 / 个人工作台；
3. AI 角色 / AI 宠物等低门槛互动作品。

更新时间：2026-08-31
